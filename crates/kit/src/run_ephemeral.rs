    ...
    for path in &paths {
        match std::fs::read_to_string(path) {
            Ok(content) => {
                ...
                // Filter out localhost, IPv6, link-local, and private network addresses
                // QEMU runs in user networking mode (slirp) inside a container, which cannot
                // reach private network addresses (10.x.x.x, 172.16-31.x.x, 192.168.x.x).
                // These are often VPN-only DNS servers that won't work from inside QEMU.
...
"#;
    ...
    if let Some(dns_servers) = opts.host_dns_servers.clone() {
        ...
        let written_content = std::fs::read_to_string("/etc/resolv.conf")
            .context("Failed to read back /etc/resolv.conf after writing")?;
        debug!("/etc/resolv.conf contents: {}", written_content.trim());
    } else {
