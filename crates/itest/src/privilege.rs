...
pub fn require_root(test_name: &str, test_binary: &str, mode: DispatchMode) -> Result<Option<()>> {
    ...
    if std::env::var_os("ITEST_IN_VM").is_some() {
        anyhow::bail!("ITEST_IN_VM is set but we are not root — VM setup is broken");
    }

    let image = std::env::var("ITEST_IMAGE").map_err(|_| {
        anyhow::anyhow!(
